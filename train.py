...
def main():
    ...
    output_dir = './test_saved_models/'
    log_dir = './test_mae_mse/'
    ...
    # last checkpoint
    checkpointfile = './pre_train/checkpoint.74.pth.tar'
    ...
    if CONTINUE_TRAIN:
        net, optimizer, start_epoch = utils.load_checkpoint(net, optimizer,filename=checkpointfile)
    ...
    # Start training
    f_train_loss = open(os.path.join(log_dir,"train_loss.csv"),"w+")
    f_val_loss = open(os.path.join(log_dir, "val_loss.csv"),"w+")
    for this_epoch in range(start_epoch, end_epoch-1):
        ...
        for blob in data_loader:
            ...
            if step % disp_interval == 0:
                ...
                utils.save_results(img_data, gt_data, et_data, output_dir, fname="{}.{}.png".format(this_epoch, step))
...
# =====================================================VALIDATION=========================================================
        ...
        log_text = 'EPOCH: %d, MAE: %.1f, MSE: %0.1f' % (this_epoch, val_mae, val_mse)
        log_print(log_text, color='green', attrs=['bold'])
        # SAVE model
        is_save = False
        if val_mae <= best_mae:
        ...
        if is_save:
            save_name = os.path.join(output_dir, '{}_{}_{}.h5'.format(method, dataset_name, this_epoch))
